...
1. Rename the repo `veronica-catelog` → `veronica-web` (cosmetic, but lines things up with the BE repo naming)
2. Install `@veronica/contracts` (the shared schema package the BE intern will publish)
...
- [ ] BE intern has published `@veronica/contracts@0.1.0` to GitHub Packages (sync with them; this is the M0 milestone)
...
- Repo (on GitHub and local disk) is renamed `veronica-web`; Vercel project linkage updated
- `package.json` name is `veronica-web`
- `@veronica/contracts@0.1.0` installed
...
1. https://github.com/ketan18710/veronica-catelog (or wherever the repo is) → Settings → "Repository name" → change to `veronica-web` → Save
2. GitHub automatically sets up a redirect from the old URL

**Verification**:
- Open https://github.com/ketan18710/veronica-web → loads
- Old URL redirects
...
2. ```bash
   ...
   mv veronica-catelog veronica-web
   ```
3. Re-open the new path in your editor

**Acceptance criteria**:
- [ ] `/Users/ketanverma/Desktop/Personal/veronica-web/` exists
- [ ] Old path gone
...
> I just renamed the repo from `veronica-catelog` to `veronica-web` (GitHub + local). Update the git remote `origin` to point at `https://github.com/ketan18710/veronica-web.git`. Verify with `git remote -v` and `git fetch`. Then update `package.json` `name` field from "veronica-catelog" to "veronica-web". Run `pnpm install` to regenerate the lockfile. Run `pnpm typecheck` to confirm nothing broke.

**Verification commands**:
```bash
git remote -v
# Expect: origin  https://github.com/ketan18710/veronica-web.git

cat package.json | grep '"name"'
# Expect: "name": "veronica-web",
...
- [ ] Typecheck passes
- [ ] Commit: `chore(phase-0): rename to veronica-web`
...
1. https://vercel.com/dashboard → find project named `veronica-catelog`
2. Settings → General → "Project Name" → change to `veronica-web` → Save
3. Production URL or custom domain keeps working; preview URLs now use `veronica-web-...` prefix
...
## Task 0.5 — Install `@veronica/contracts`

**Prerequisite**: BE intern published `@veronica/contracts@0.1.0`. Confirm with them.
...
> 3. Install: `pnpm add @veronica/contracts@^0.1.0`. Should succeed if my `GITHUB_TOKEN` env var is set with `read:packages` scope.
> 4. Verify the package is in `node_modules`: `ls node_modules/@veronica/contracts/dist/` — should show index.js, index.d.ts, etc.
> 5. As a smoke test, create a temp file `/tmp/sanity.ts` with `import { CategorySchema } from "@veronica/contracts"; console.log(CategorySchema);` and run `pnpm tsx /tmp/sanity.ts` — should print the schema object. Delete the temp file after.
...
- [ ] `@veronica/contracts@^0.1.0` installed
- [ ] Sanity import works
- [ ] Commit: `chore(phase-0): install @veronica/contracts`
...
>
> 2. `src/mocks/data/categories.ts` — export an array of 8 categories matching the seed data (4 roots: kitchen-sinks, health-faucet-sets, bathroom-accessories, plumbing-fittings + 4 subcategories under Kitchen Sinks and Health Faucet Sets). Match `CategorySchema` shape from `@veronica/contracts`.
>
