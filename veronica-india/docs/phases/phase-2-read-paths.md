...
Swap every catalog read on the storefront from the in-memory `src/lib/data.ts` to fetch calls against the real `veronica-api`. After this phase, deleting `data.ts` doesn't break anything.
...
- [ ] Backend staging URL known (e.g. `https://veronica-api-staging.workers.dev` or `https://veronica-api.fly.dev`)
...
## Task 1.1 — Update `@veronica/contracts` and extend `src/lib/backend.ts`
...
**Files to touch**:
- `package.json` — bump `@veronica/contracts` to `^0.1.0`
- `src/lib/backend.ts` — add methods for new endpoints
- `.env.local` — set `NEXT_PUBLIC_API_URL` to the staging URL

**Suggested Claude Code prompt**:
> Bump `@veronica/contracts` to `^0.1.0` in `package.json`, run `pnpm install`. Verify the new schemas (`CategoryWithBreadcrumbSchema`, `ProductSchema`, `ProductListItemSchema`) are exported from `@veronica/contracts`.
>
...
# /product/veronica-brass-health-faucet-set (no dimensions, single SKU)
