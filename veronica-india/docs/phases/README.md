...
Execution playbooks for the `veronica-web` repo (the Next.js storefront + admin). Each phase is a self-contained set of tasks an intern can complete with Claude Code.

## How development works across the two repos

The frontend and backend develop **in parallel, independently**. You don't wait for the backend intern. Instead, you use **MSW (Mock Service Worker)** to fake all API responses against the schemas exported by `@veronica/contracts`. Your UI develops as if the backend exists.
...
1. `pnpm add @veronica/contracts@latest`
