...
| **Type sharing** | `@veronica/contracts` package | Lives inside `veronica-api` repo, published to GitHub Packages under `ketan18710`. Versions zod schemas + inferred TS types. |
| **Repo layout** | Two repos: `veronica-web` + `veronica-api` | User preference. Types shared via the contracts package. |
...
- We mitigate the cost via the `@veronica/contracts` package — schemas + types live in one place and both repos depend on it.
