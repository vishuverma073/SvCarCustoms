# Veronica Frontend — Backend Integration Runbook
...
>
> Owner split: **we own the frontend**; a separate dev owns the backend (`veronica-api-`). Last updated: 2026-05-31 (after Phase 5).
...
- **Now:** local zod schemas in `src/contracts/*`, exposed as `@veronica/contracts` via a **tsconfig path alias** ([tsconfig.json](tsconfig.json) `paths`).
- **At integration:** `npm i @veronica/contracts`, then **delete the alias + the `src/contracts/` folder**. Module resolution falls through to the package — zero import-site edits. TypeScript then flags any drift between our assumed shapes and the real package at compile time.
...
| Refresh | **localStorage marker** `veronica-mock-refresh` (the phone), replayed to `/auth/refresh` because a cross-origin httpOnly cookie can't be simulated. See `MOCK_REFRESH_MARKER` in [backend.ts](src/lib/backend.ts) | real **httpOnly refresh cookie** (`credentials:"include"` is already set). **Delete** writeMarker/readMarker/clearMarker + the `USE_MOCKS ? {phone} : {}` branch in `doRefresh()` |
...
The exact contract every mock handler answers (paths relative to `NEXT_PUBLIC_API_URL`). Shapes = the `@veronica/contracts` schemas.
...
- [ ] **Delete orphaned Drizzle prototype** `src/db/{index,schema,seed}.ts` + `data/veronica.db` (dead — nothing imports it; backend owns the real DB)
