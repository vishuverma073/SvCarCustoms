...
- `@veronica/contracts@0.1.0` published with the new schemas
...
> 5. Return shape matching `CategoryWithBreadcrumbSchema` from `@veronica/contracts`
...
> 4. Compose the response matching `ProductSchema` from `@veronica/contracts`
...
# Expect: 3 or 4 (Lavender + Jasmine + Veronica Drain Board, possibly more)
...
>    ```toml
>    name = "veronica-api-staging"
>    main = "src/entry/workers.ts"
>    compatibility_date = "2024-08-01"
>    compatibility_flags = ["nodejs_compat"]
>    ```
>
> 4. Document how to set secrets: `wrangler secret put DATABASE_URL --name veronica-api-staging`. The DATABASE_URL for Workers should use Supabase's pooler URL in transaction mode.
>
...
>    - Copy source, run `pnpm --filter @veronica/api build`
...
curl -s https://veronica-api-staging.<host>.workers.dev/healthz
# OR
curl -s https://veronica-api.fly.dev/healthz
...
## Task 1.8 — Bump and publish `@veronica/contracts@0.1.0`
...
> 4. Verify visible: `pnpm view @veronica/contracts version --registry https://npm.pkg.github.com`
...
**Acceptance criteria**:
- [ ] `@veronica/contracts@0.1.0` published
- [ ] Git tag pushed
