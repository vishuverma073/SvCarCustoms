...
This is the spec for the admin panel. It applies to both the backend API shapes and the frontend UI in [veronica-web/docs/admin-design.md](../../veronica-catelog/docs/admin-design.md).
...
Extends the existing storefront tokens from [src/app/globals.css](../../veronica-catelog/src/app/globals.css):
...
Home page reads `home_config` (cached, see Phase 5). Each section honors `enabled` + `order`. Featured section uses the curated `productIds`. The existing storefront home page ([src/app/(store)/page.tsx](../../veronica-catelog/src/app/(store)/page.tsx)) will be rebuilt in Phase 2 to render from this config rather than hardcoded sections.
