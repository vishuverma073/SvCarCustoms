...
describe("admin mock handlers", () => {
  ...
  it("filters products by categoryTreeId and includes category counts", async () => {
    ...
    const bodyKits = cats.find((c) => c.slug === "body-kits");
    expect(bodyKits?.subtreeProductCount).toBeGreaterThan(0);

    const res = await fetch(`${API_BASE}/admin/products?categoryTreeId=${bodyKits!.id}`, {
      headers: AUTH,
    });
    const body = await res.json();
    expect(body.items.length).toBe(bodyKits!.subtreeProductCount);
