...
describe("backend products", () => {
  ...
  it("filters by flag and by category subtree", async () => {
    ...
    const bodyKits = await backend.getProductsByCategory("body-kits");
    expect(bodyKits.length).toBeGreaterThan(0);
  });

  it("includes sizes on category product lists for filtering", async () => {
    const items = await backend.getProductsByCategory("speakers");
    expect(items.length).toBeGreaterThan(0);
    const withSizes = items.filter((p) => p.sizes.length > 0);
    expect(withSizes.length).toBeGreaterThan(0);
    expect(withSizes[0].sizes).toContain('5.25"');
  });

  it("searches by free text", async () => {
    const results = await backend.searchProducts("exhaust");
    expect(results.length).toBeGreaterThan(0);
    expect(
      results.every((p) => /exhaust/i.test(p.name) || p.tags.some((t) => /exhaust/i.test(t))),
    ).toBe(true);
  });
