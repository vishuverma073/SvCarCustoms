...
export default function ProductEditor({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-2xl pb-36 lg:pb-24 space-y-3">
      ...
      {/* Vehicle fitment */}
      <Section title="Vehicle Fitment" defaultOpen>
        <FitmentEditor
          fitsAllVehicles={fitsAllVehicles}
          fitments={fitments}
          onChange={(fitsAll, next) => {
            setValue("fitsAllVehicles", fitsAll, { shouldDirty: true });
            setValue("fitments", next, { shouldDirty: true });
          }}
        />
      </Section>
