...
Gate criteria between phases. Same content as `svcar-api/docs/integration-milestones.md` — keeping a copy here so the FE intern doesn't have to flip repos.

## How a milestone works (frontend's responsibilities)

1. **Wait for BE signal**: contracts version bumped, BE staging deployed.
2. **Update mocks**: `pnpm add @svcar/contracts@latest`, fix any new TS errors, update MSW handlers to match new schema shape.
...
```
NEXT_PUBLIC_API_URL=https://svcar-api-staging.workers.dev
