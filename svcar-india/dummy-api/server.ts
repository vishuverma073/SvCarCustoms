/**
 * Svcar DUMMY backend — a real HTTP server for local testing.
 *
 * It reuses the EXACT same MSW request handlers + seed data the frontend mocks
 * use (so every response shape matches the `@svcar/contracts` schemas the FE
...
const REFRESH_COOKIE = "svcar_refresh";
...
app.listen(PORT, HOST, () => {
  console.log(`\n  Svcar dummy backend → http://${HOST}:${PORT}`);
  if (HOST === "0.0.0.0") {
