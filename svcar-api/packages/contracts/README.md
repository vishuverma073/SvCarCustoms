...
Shared **zod schemas** and inferred **TypeScript types** that define the contract between the Svcar API (`@svcar/api`) and the web client (`svcar-web`).
...
Every breaking change needs a major bump **and** a heads-up to the FE intern in `#svcar-dev` before publishing.
...
```ts
import { CategorySchema, type Category } from "@svcar/contracts";
...
Published to GitHub Packages under the `@svcar` scope. Every schema change is a **version bump** that the frontend must install — announce it in `#svcar-dev` before publishing.
