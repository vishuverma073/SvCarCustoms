...
export const products = pgTable(
  "products",
  {
    ...
    // Vehicle fitment: universal accessories (true) fit every car and carry no
    // product_fitments rows; vehicle-specific parts set this false and declare
    // their compatible vehicles in product_fitments.
    fitsAllVehicles: boolean("fits_all_vehicles").default(true).notNull(),
    ...
    // svcar_product_search(), created in migration 0003 (Drizzle can't emit
    // the function itself).
    searchVector: tsvector("search_vector").generatedAlwaysAs(
      sql`svcar_product_search(name, description, tags)`,
    ),
  },
  (t) => [
...
export const vehicleMakes = pgTable(
  ...
  (t) => [index("vehicle_makes_sort_idx").on(t.sortOrder)],
);
...
export const vehicleModels = pgTable(
  ...
  (t) => [index("vehicle_models_make_id_idx").on(t.makeId)],
);

/** Compatible vehicles for a non-universal product (make/model + optional year range). */
export const productFitments = pgTable(
  ...
  (t) => [
    index("product_fitments_product_id_idx").on(t.productId),
    index("product_fitments_make_model_idx").on(t.make, t.model),
  ],
);
...
export const settings = pgTable(
  ...
  {
    id: integer("id").primaryKey().default(1),
    storeName: text("store_name").notNull().default("SV Car Customs"),
    supportPhone: text("support_phone").notNull().default("+919350529717"),
    supportEmail: text("support_email").notNull().default("support@svcarcustoms.com"),
    storeAddress: jsonb("store_address").$type<StoreAddress>().notNull(),
    gstRate: numeric("gst_rate", { precision: 4, scale: 2 }).notNull().default("28.00"),
...
export const productFitmentsRelations = relations(productFitments, ({ one }) => ({
  product: one(products, {
    fields: [productFitments.productId],
