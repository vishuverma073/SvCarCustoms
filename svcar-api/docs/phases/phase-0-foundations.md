...
End state of Phase 0: a clean Hono service runs locally against real Postgres, returns seeded categories from the database, has green tests + CI, and the FE intern can install `@svcar/contracts@0.1.0` and start building.
...
- [ ] GitHub account `ketan18710` (or your own) with permission to create the `svcar-api` repo
...
- GitHub repo exists at `github.com/ketan18710/svcar-api` with CI passing
...
The FE intern is doing their own Phase 0 (renaming the repo, installing MSW, setting up mocks). The main handoff: once you publish `@svcar/contracts@0.1.0`, ping them — they can install it and proceed.
...
**Files to create** (at the root of `/Users/ketanverma/Desktop/Personal/svcar-api/`):
...
> I'm at `/Users/ketanverma/Desktop/Personal/svcar-api/`. Set up a pnpm workspace.
>
> 1. Create `package.json` at the root:
>    - name: "svcar-api-workspace"
>    - version: "0.0.0"
>    - private: true
>    - packageManager: "pnpm@9.0.0"
>    - engines: node >=20.11.0
>    - scripts: `build`, `dev`, `test`, `typecheck`, `db:generate`, `db:push`, `db:migrate`, `db:studio`, `db:seed` — each delegating to `pnpm --filter @svcar/api ...`
>
...
```bash
cd /Users/ketanverma/Desktop/Personal/svcar-api
pnpm install
...
## Task 0.2 — Set up `@svcar/contracts` package

**Context**: Shared zod schemas + TS types. Published to GitHub Packages; consumed by both this repo (via workspace protocol) and `svcar-web` (via real npm install).
...
**Suggested Claude Code prompt**:
> Create the `@svcar/contracts` package.
>
> 1. `packages/contracts/package.json`:
>    - name: `@svcar/contracts`, version `0.0.1`, type `module`
...
>    - repository: { type: git, url: https://github.com/ketan18710/svcar-api.git, directory: packages/contracts }
>
...
> 10. Build and verify:
>    ```bash
>    pnpm --filter @svcar/contracts build
...
pnpm --filter @svcar/contracts typecheck
# Expect: no errors
```
...
- [ ] Typecheck passes
- [ ] Commit: `feat(phase-0): @svcar/contracts package skeleton`

---

## Task 0.3 — Set up `@svcar/api` package (Hono skeleton)
...
**Suggested Claude Code prompt**:
> Create the `@svcar/api` package.
>
> 1. `apps/api/package.json`:
>    - name `@svcar/api`, version `0.0.1`, type `module`, private true
...
>    - dependencies: `@hono/node-server` ^1.11, `@svcar/contracts` workspace:*, `drizzle-orm` ^0.45, `hono` ^4.7, `postgres` ^3.4, `zod` ^3.23
...
>    ```
>    DATABASE_URL=postgresql://postgres:postgres@localhost:5432/svcar
>    PORT=8787
>    LOG_LEVEL=info
...
```bash
pnpm install
pnpm --filter @svcar/api typecheck
# Expect: no errors
...
pnpm dev
# Expect: "svcar-api listening on http://localhost:8787"
...
**Context**: This is the canonical Postgres schema, modeled on the in-memory `svcar-catelog/src/lib/data.ts` shape (which has richer types than the old `src/db/schema.ts` in that repo — do NOT use the old schema). Phase 1 adds a few admin-specific columns + tables on top of this baseline.
...
>
> Map each DB row to the `CategorySchema` shape from `@svcar/contracts`. Validate the response with `CategoryListSchema.parse()` before sending.
>
...
**Context**: The existing `svcar-catelog/src/lib/data.ts` has 8 categories + 12 products with full variant data. We need this in Postgres.
...
**Files to create**:
- `apps/api/scripts/seed-data.ts` — fixture, mirrors the shape from svcar-catelog data.ts
- `apps/api/scripts/seed-from-data.ts` — the actual seeder

**Suggested Claude Code prompt**:
> Create the seed script.
>
> 1. Read `/Users/ketanverma/Desktop/Personal/svcar-catelog/src/lib/data.ts` (this is the FE repo). Extract the `categories` and `products` arrays.
>
...
>   5. `pnpm --filter @svcar/contracts build`
>   6. `pnpm typecheck`
>   7. `pnpm test`
...
## Task 0.11 — Push to GitHub + publish `@svcar/contracts@0.1.0`

**What to do** (mix of manual + Claude):

1. **Manual**: https://github.com/new → owner `ketan18710`, name `svcar-api`, Private, no README/gitignore/license (we have them).
2. **Claude prompt**:
   > I'm at `/Users/ketanverma/Desktop/Personal/svcar-api`. Initialize git if not already (`git init`). Stage all files except `.env` and `node_modules` (already gitignored). Commit `chore: initial scaffold` with the Co-Authored-By line for Claude. Add remote `origin` as `https://github.com/ketan18710/svcar-api.git`. Push `main`. Then verify CI ran by checking `gh pr` after opening a no-op PR.
...
4. **Publish contracts**:
   ```bash
   pnpm --filter @svcar/contracts build
   pnpm --filter @svcar/contracts publish --access restricted --no-git-checks
   pnpm view @svcar/contracts version --registry https://npm.pkg.github.com
   # Expect: 0.1.0
   ```
5. **Notify FE intern**: "@svcar/contracts@0.1.0 is published. You can install it."
