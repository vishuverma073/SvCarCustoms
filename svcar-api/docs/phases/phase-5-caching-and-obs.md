...
1. BetterStack Uptime → Add monitor:
   - URL: `https://api.svcarindia.com/healthz` (or staging URL)
   - Interval: 60s
   - Alert: if down ≥ 2 min, SMS + email to Ketan
2. Add monitor for the Razorpay webhook endpoint:
   - URL: `https://api.svcarindia.com/webhooks/razorpay-health` (you'll need to add a GET handler that returns 200 OK without doing anything — purely for monitoring)
3. Add monitor for the frontend:
   - URL: `https://svcarindia.com`
   - Same alert rules
